#include "include/cpu/cpu_opt9.h"
...
static void gemm_cpu_per_thread(const thread_param_t& param){
...
template<typename T>
void CpuOpt9<T>::gemm(bool transA, bool transB, int m, int n, int k, \
        T* a, int lda, T* b, int ldb, T* c, int ldc, T alpha, T beta){
    printf("Unimplemented!\n");
}

template<>
void CpuOpt9<float>::gemm(bool transA, bool transB, int m, int n, int k, \
        float* a, int lda, float* b, int ldb, float* c, int ldc, float alpha, float beta){
    if(m % m_block != 0 || n % n_block != 0 || 0 == m || 0 == n){
        printf("Optimize9: Param m = %d, n = %d not implemented!\n", m, n);
        return;
    }
...
template<typename T>
void CpuOpt9<T>::operator()(bool transA, bool transB, int m, int n, int k, \
        T* a, int lda, T* b, int ldb, T* c, int ldc, T alpha, T beta){
...
template class CpuOpt9<float>;

// register CPU_RAW to GEMM Repo;
REGISTER_GEMM(CPU_OPT9, CpuOpt9);