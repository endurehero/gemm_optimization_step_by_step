#include "include/cpu/cpu_opt8.h"
...
template<typename T>
void CpuOpt8<T>::gemm(bool transA, bool transB, int m, int n, int k, \
        T* a, int lda, T* b, int ldb, T* c, int ldc, T alpha, T beta){
    printf("Unimplemented!\n");
}

template<>
void CpuOpt8<float>::gemm(bool transA, bool transB, int m, int n, int k, \
        float* a, int lda, float* b, int ldb, float* c, int ldc, float alpha, float beta){
    if(m % m_block != 0 || n % n_block != 0 || 0 == m || 0 == n){
        printf("Optimize8: Param m = %d, n = %d not implemented!\n", m, n);
        return;
    }
...
template<typename T>
void CpuOpt8<T>::operator()(bool transA, bool transB, int m, int n, int k, \
        T* a, int lda, T* b, int ldb, T* c, int ldc, T alpha, T beta){
...
template class CpuOpt8<float>;

// register CPU_RAW to GEMM Repo;
REGISTER_GEMM(CPU_OPT8, CpuOpt8);