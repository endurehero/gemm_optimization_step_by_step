#include "include/cpu/cpu_opt5.h"
...
template<typename T>
void CpuOpt5<T>::gemm(bool transA, bool transB, int m, int n, int k, \
...
template<typename T>
void CpuOpt5<T>::operator()(bool transA, bool transB, int m, int n, int k, \
        T* a, int lda, T* b, int ldb, T* c, int ldc, T alpha, T beta){
...
template class CpuOpt5<float>;

// register CPU_RAW to GEMM Repo;
REGISTER_GEMM(CPU_OPT5, CpuOpt5);